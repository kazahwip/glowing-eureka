...
exports.buildModelCardText = buildModelCardText;
exports.buildModelSafetyPolicyText = buildModelSafetyPolicyText;
exports.buildModelCertificateText = buildModelCertificateText;
exports.buildModelReviewsText = buildModelReviewsText;
exports.buildScheduleText = buildScheduleText;
...
function buildModelReviews(card) {
...
function buildTodaySchedule(card) {
    ...
    return [
        `<b>📅 РАСПИСАНИЕ ${(0, text_1.escapeHtml)(card.name.toUpperCase())} НА СЕГОДНЯ</b>`,
        "",
        "<b>🟢 СВОБОДНО:</b>",
        ...freeSlots.map((slot) => `• ${(0, text_1.escapeHtml)(slot)}`),
        "",
        "<b>🔴 ЗАНЯТО:</b>",
        `• ${(0, text_1.escapeHtml)(busySlot)}`,
        "",
        "<b>⚡ ГОРЯЧЕЕ ПРЕДЛОЖЕНИЕ:</b>",
        `Слот ${(0, text_1.escapeHtml)(stats.hotOfferSlot)} со скидкой ${stats.hotOfferDiscount}%!`,
...
function buildWeekSchedule(card) {
    const random = createRandom(`week:${card.id}:${card.name}`);
    const lines = [`<b>📅 РАСПИСАНИЕ ${(0, text_1.escapeHtml)(card.name.toUpperCase())} НА НЕДЕЛЮ</b>`, ""];
    weekDayLabels.forEach((label, index) => {
        ...
        [...used]
            ...
            lines.push(`• ${(0, text_1.escapeHtml)(todaySlots[slotIndex])} -${discount}%`);
        });
        lines.push("");
    });
    return lines.join("\n");
}
function buildModelCardText(card) {
    const stats = buildStats(card);
    return [
        ...
        "",
        "<b>🔐 УСЛОВИЯ:</b>",
        "• Первое бронирование подтверждается депозитом от 5,000 RUB.",
        "• Средства резервируются платформой до подтверждения завершения встречи.",
        "• После успешного первого заказа доступны прямые взаиморасчеты наличными.",
        "",
        "<b>✅ ГАРАНТИИ:</b>",
        "• Верифицированные данные профиля доступны по запросу.",
        "• При несоответствии профиля условиям бронирование можно отменить через поддержку.",
        "",
...
function buildModelSafetyPolicyText() {
    return [
<b>🏛 ПОЛИТИКА БЕЗОПАСНОСТИ И ПЕРВОЙ ВСТРЕЧИ</b>,
...
"Агентство Honey Bunny создано для организации эскорт-сопровождения и ставит безопасность моделей превыше всего.",
"Мы несем полную ответственность за верификацию анкет, медицинскую чистоту и конфиденциальность.",

"",

"<b>Условия первой встречи:</b>",
"✅ Вы выбираете любую сумму бронирования от 5.000₽ в прайсе модели.",
"✅ Средства зачисляются на транзитный депозит агентства как СТРАХОВОЙ ДЕПОЗИТ БЕЗОПАСНОСТИ.",
"✅ Модель НЕ ПОЛУЧАЕТ деньги до тех пор, пока не нажмет кнопку Завершить встречу и не покинет адрес.",
"✅ Вы имеете право запросить медкнижку и скан паспорта модели ДО внесения оплаты.",

"",

"<b>Последующие встречи:</b>",
"После успешно завершенной первой встречи вы получаете статус ДОВЕРЕННЫЙ ГОСТЬ.",
"Начиная со второй встречи вы можете полностью перейти на НАЛИЧНЫЙ РАСЧЕТ напрямую с моделью.",

"",

"<b>Гарантия подмены:</b>",
"Если девушка при личной встрече не соответствует фотографиям в анкете, вы нажимаете кнопку ОТКАЗ.",
"Агентство мгновенно возвращает 100% суммы брони + 20% компенсации за испорченное время на вашу карту.",

"",

"Служба заботы Honey Bunny работает круглосуточно.",
"Это официальная политика агентства. Нажмите ПРИНЯТЬ для продолжения."
    ].join("\n");
}
function buildModelCertificateText(card) {
    const stats = buildStats(card);
    return [
...
function buildModelReviewsText(card, page = 1, limit = 3) {
    const stats = buildStats(card);
    const reviews = buildModelReviews(card);
    const offset = Math.max(0, (page - 1) * limit);
    const pageItems = reviews.slice(offset, offset + limit);
    const lines = [
        `<b>⭐ ОТЗЫВЫ О ${(0, text_1.escapeHtml)(card.name.toUpperCase())}</b>`,
        "",
    ...
    pageItems.forEach((review, index) => {
        lines.push(`<b>${offset + index + 1}. ${(0, text_1.escapeHtml)(review.author)}</b>`, `⭐ 5/5 • ${(0, text_1.escapeHtml)(review.whenLabel)}`, `"${(0, text_1.escapeHtml)(review.text)}"`, "");
    });
    return {
        text: lines.join("\n").trim(),
        hasPrev: page > 1,
...
function buildScheduleText(card, mode) {
    return mode === "week" ? buildWeekSchedule(card) : buildTodaySchedule(card);
}
function buildPrebookingText() {
    return [
...
function buildGeneratedFeedItem(card, review) {
    const stats = buildStats(card);
    return {
        createdAt: review.createdAt,
        html: [
            `<b>⭐ ${(0, text_1.escapeHtml)(card.name)} · ${(0, text_1.escapeHtml)(card.city)}</b>`,
            `Рейтинг: ${stats.rating}/5.0`,
            `Положительные отзывы: ${stats.positivePercent}%`,
            "",
            `<b>${(0, text_1.escapeHtml)(review.author)}</b> • ${(0, text_1.escapeHtml)(review.whenLabel)}`,
            (0, text_1.escapeHtml)(review.text),
        ].join("\n"),
    };
}
async function listReviewFeed(page = 1, limit = 5) {
    const db = await (0, client_1.getDb)();
    const cards = await db.all("SELECT * FROM cards WHERE is_active = 1 ORDER BY created_at DESC LIMIT 80");
    const storedReviews = await db.all(`SELECT reviews.text, reviews.created_at, users.username, users.first_name
     FROM reviews
     JOIN users ON users.id = reviews.user_id
     ORDER BY reviews.created_at DESC
     LIMIT 40`);
    const generatedItems = cards.flatMap((card) => buildModelReviews(card)
        .slice(0, 2)
        .map((review) => buildGeneratedFeedItem(card, review)));
    const storedItems = storedReviews.map((item) => ({
        createdAt: Date.parse(item.created_at),
        html: [
            "<b>⭐ Отзыв клиента (Измененные в целях соблюдения политики Telegram)</b>",
            `${(0, text_1.escapeHtml)(item.username ? `@${item.username}` : item.first_name || "Гость сервиса")}`,
            "",
            (0, text_1.escapeHtml)(item.text),
        ].join("\n"),
    }));
