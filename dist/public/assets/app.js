...
async function renderCatalog() {
    ...
    <div class="shell">
      ...
      <section class="section">
        <div class="tabs">
          ${data.categories
        .map((item) => `<button class="tab${item.key === state.category ? "" : " secondary"}" data-category="${item.key}">${escapeHtml(item.label)}</button>`)
        .join("")}
        </div>
...
async function renderCard() {
  ...
  `;
    ...
    document.querySelector("#schedule-btn")?.addEventListener("click", async () => {
        const schedule = await api(`/api/webapp/cards/${card.id}/schedule?mode=today`);
...
async function bootstrap() {
    ...
    state.screen = (requestedScreen && requestedScreen in sectionLabels ? requestedScreen : state.bootstrap?.initialScreen ?? "catalog");
